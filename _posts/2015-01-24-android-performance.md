...
![Android Performance Patterns](/assets/android_performance_overview.jpg)
...
![60fps](/assets/android_performance_60fps.jpg)

如果你的某个操作花费时间是24ms，系统在得到VSYNC信号的时候就无法进行正常渲染，这样就发生了丢帧现象。那么用户在32ms内看到的会是同一帧画面。

![Drop frames](/assets/android_performance_drop_frames.jpg)
...
![Overdraw](/assets/android_performance_overdraw.jpg)
...
![Android Overdraw](/assets/android_performance_android_overdraw.jpg)
...
![Profile GPU Rendering](/assets/android_performance_profile_gpu.jpg)

选择了这样以后，我们可以在手机画面上看到丰富的GPU绘制图形信息，分别关于StatusBar，NavBar，激活的程序Activity区域的GPU Rending信息。

![Active App Performance](/assets/android_performance_profile_active_app.jpg)

随着界面的刷新，界面上会滚动显示垂直的柱状图来表示每帧画面所需要渲染的时间，柱状图越高表示花费的渲染时间越长。

![Frame Time](/assets/android_performance_profile_frame_time.jpg)

中间有一根绿色的横线，代表16ms，我们需要确保每一帧花费的总时间都低于这条横线，这样才能够避免出现卡顿的问题。

![Frame Time Component](/assets/android_performance_profile_frame_time_component.jpg)
...
![Resterization](/assets/android_performance_resterization.jpg)

Resterization栅格化是绘制那些Button，Shape，Path，String，Bitmap等组件最基础的操作。它把那些组件拆分到不同的像素上进行显示。这是一个很费时的操作，GPU的引入就是为了加快栅格化的操作。

CPU负责把UI组件计算成Polygons，Texture纹理，然后交给GPU进行栅格化渲染。

![Resterization Progress](/assets/android_performance_resterization_progress.jpg)
...
![Rending Progress](/assets/android_performance_rending_progress.jpg)
...
![Customized View](/assets/android_performance_api_customize.jpg)
...
![Clip Rect](/assets/android_performance_api_clipRect.jpg)
...
![Heap Memory](/assets/android_performance_heap_memory.jpg)

除了速度差异之外，执行GC操作的时候，任何线程的任何操作都会需要暂停，等待GC操作完成之后，其他操作才能够继续运行。

![GC time](/assets/android_performance_gc_time.jpg)
...
![Memory Churn](/assets/android_performance_memory_churn.jpg)

解决上面的问题有简洁直观方法，如果你在Memory Monitor里面查看到短时间发生了多次内存的涨跌，这意味着很有可能发生了内存抖动。

![Memory Monitor](/assets/android_performance_memory_monitor.jpg)
...
![Object Generation](/assets/android_performance_object_generation.jpg)

每一个级别的内存区域都有固定的大小，此后不断有新的对象被分配到此区域，当这些对象总的大小快达到这一级别内存区域的阀值时，会触发GC的操作，以便腾出空间来存放其他新的对象。

![GC Trigger](/assets/android_performance_gc_trigger.jpg)
...
![Activity Release](/assets/android_performance_activity_release.jpg)
...
![Track Allocation](/assets/android_performance_track_allocation.jpg)
...
![Device & Activity](/assets/android_performance_device_activity.jpg)

![Free & Allocated](/assets/android_performance_free_allocated.jpg)

![GC Event](/assets/android_performance_gc_event.jpg)
...
![Battery Static](/assets/android_performance_battery_static.jpg)
...
![N5 Cellular Radio](/assets/android_performance_cellular_radio.jpg)
...
![Battery Drain & WakeLock](/assets/android_performance_drain_wakelock.jpg)
